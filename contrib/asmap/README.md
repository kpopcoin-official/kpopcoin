...
be used in Kpopcoin Core with the `-asmap` option.
...
to binary before being used with Kpopcoin Core.
...
the `kpopcoin-cli getnodeaddresses` command.
...
```
kpopcoin-cli getnodeaddresses 0 > addrs.json
```
