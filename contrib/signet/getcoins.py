...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing kpopcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='kpopcoin-cli', help='kpopcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Kpopcoin address to which the faucet should send')
...
parser.add_argument('kpopcoin_cli_args', nargs='*', help='Arguments to pass on to kpopcoin-cli (default: -signet)')

args = parser.parse_args()

if args.kpopcoin_cli_args == []:
    args.kpopcoin_cli_args = ['-signet']


def kpopcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.kpopcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = kpopcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = kpopcoin_cli(['getnewaddress', 'faucet', 'bech32'])
