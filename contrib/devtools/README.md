...
gen-kpopcoin-conf.sh
===================

Generates a kpopcoin.conf file in `share/examples/` by parsing the output from `kpopcoind --help`. This script is run during the
release process to include a kpopcoin.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-kpopcoin-conf.sh
```
