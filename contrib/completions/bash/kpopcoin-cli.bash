# bash programmable completion for kpopcoin-cli(1)
# Copyright (c) 2012-present The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $kpopcoin-cli for RPC
_kpopcoin_rpc() {
...
_kpopcoin_cli() {
    ...
    # save and use original argument to invoke kpopcoin-cli for -help, help and RPC
    # as kpopcoin-cli might not be in $PATH
    kpopcoin_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($kpopcoin_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_kpopcoin_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _kpopcoin_cli kpopcoin-cli
