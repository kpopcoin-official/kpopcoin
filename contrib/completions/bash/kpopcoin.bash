# bash programmable completion for kpopcoin(1) wrapper
...
_kpopcoin() {
    ...
    local kpopcoin subcmd offset

    # save and use original argument to invoke kpopcoin for help
    ...
    case "$subcmd" in
        gui|node)
            _kpopcoin_wrap kpopcoind "$offset"
            return 0
            ;;
        rpc)
            _kpopcoin_wrap kpopcoin-cli "$offset"
            return 0
            ;;
        tx)
            _kpopcoin_wrap kpopcoin-tx "$offset"
            return 0
            ;;
    esac
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                options=$($kpopcoin help 2>&1 | awk '{ for(i=1;i<=NF;i++) if ($i~/^--/) { sub(/=.*/, "=",$i); print $i } }' )
            fi
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$($kpopcoin help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
