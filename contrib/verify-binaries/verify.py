...
"""Script for verifying Kpopcoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from kpopcoincore.org and kpopcoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/kpopcoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://kpopcoincore.org"
HOST2 = "https://kpopcoin.org"
VERSIONPREFIX = "kpopcoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Kpopcoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"kpopcoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by kpopcoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the kpopcoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes kpopcoin.org lags behind kpopcoincore.org.)')
    )
