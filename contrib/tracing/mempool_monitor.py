...
""" Example logging Kpopcoin Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into kpopcoind with pid {pid}")
    kpopcoind_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    kpopcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    kpopcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    kpopcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    kpopcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[kpopcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of kpopcoind>")
        exit(1)
