# Bootstrappable Kpopcoin Core Builds

This directory contains the files necessary to perform bootstrappable Kpopcoin
Core builds.
...
To build Kpopcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Kpopcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/kpopcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/kpopcoin-core/guix.sigs).
