...
This guide describes how to build kpopcoind, command-line utilities, and GUI on Windows using Microsoft Visual Studio.
...
The Kpopcoin Core project supports the following vcpkg triplets:
- `x64-windows` (both CRT and library linkage is dynamic)
- `x64-windows-static` (both CRT and library linkage is static)

To facilitate build process, the Kpopcoin Core project provides presets, which are used in this guide.
...
vcpkg installation during the configuration step might fail for various reasons unrelated to Kpopcoin Core.
...
can occur if your local Kpopcoin Core repository path contains spaces, you can override the vcpkg install directory
...
To improve the build process performance, one might add the Kpopcoin repository directory to the Microsoft Defender Antivirus exclusions.
