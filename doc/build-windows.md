...
Below are some notes on how to build Kpopcoin Core for Windows.

The options known to work for building Kpopcoin Core on Windows are:
...
Note that for WSL the Kpopcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/kpopcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\kpopcoin`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/kpopcoin
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/kpopcoin --strip
```
