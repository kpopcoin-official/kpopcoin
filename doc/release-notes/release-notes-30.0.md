...
Kpopcoin Core version v30.0 is now available from:

  <https://kpopcoincore.org/bin/kpopcoin-core-30.0/>
...
installer (on Windows) or just copy over `/Applications/Kpopcoin-Qt` (on macOS)
...
Kpopcoin Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. Kpopcoin
...
New `kpopcoin` command
---------------------

- A new `kpopcoin` command line tool has been added to make features more discoverable
  and convenient to use. The `kpopcoin` tool just calls other executables and does not
  implement any functionality on its own. Specifically `kpopcoin node` is a synonym for
  `kpopcoind`, `kpopcoin gui` is a synonym for `kpopcoin-qt`, and `kpopcoin rpc` is a synonym
  for `kpopcoin-cli -named`. Other commands and options can be listed with `kpopcoin help`.
  The new `kpopcoin` command is an alternative to calling other commands directly, but it
...
- The new `kpopcoin` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `kpopcoin -m node -ipcbind=unix` it will
  ...
  (`kpopcoin-node` instead of `kpopcoind`) and is currently required but will become optional
  in the future (with [#33229](https://github.com/kpopcoin/kpopcoin/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md](https://github.com/kpopcoin/kpopcoin/blob/master/doc/multiprocess.md)),
...
- The `test_kpopcoin` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `kpopcoin` command
  as `kpopcoin test`. The `libexec/` directory also contains new `kpopcoin-node` and
  `kpopcoin-gui` binaries which support IPC features and are called through the
  `kpopcoin` tool. In source builds only, `test_kpopcoin-qt`, `bench_kpopcoin`, and
  `kpopcoin-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `kpopcoin` command. See `kpopcoin help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of Kpopcoin Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in Kpopcoin Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the kpopcoin-wallet tool,
...
[Transifex](https://explore.transifex.com/kpopcoin/kpopcoin/).
