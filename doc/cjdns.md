# CJDNS support in Kpopcoin Core

It is possible to run Kpopcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Kpopcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Kpopcoin network and individual nodes.
...
## Run Kpopcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Kpopcoin Core
...
When enabled, this option tells Kpopcoin Core that it is running in an
...
IPv6 local network. This helps Kpopcoin Core perform better address management:
...
CJDNS support was added to Kpopcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `kpopcoin-cli -addrinfo` to see the
...
## CJDNS-related information in Kpopcoin Core

There are several ways to see your CJDNS address in Kpopcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `kpopcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `kpopcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `kpopcoin-cli help getnodeaddresses` for details.
