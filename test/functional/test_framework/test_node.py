...
"""Class for kpopcoind node under test"""
...
KPOPCOIND_PROC_WAIT_TIMEOUT = 60
...
# 'kpopcoin' command, it may internally insert more args, which must be
...
KPOPCOIN_PID_FILENAME_DEFAULT = "kpopcoind.pid"
...
class TestNode():
    """A class for representing a kpopcoind node under test.
    ...
    def __init__(
        ...
        *,
        ...
        self.kpopcoinconf = self.datadir_path / "kpopcoin.conf"
        ...
        # Configuration for logging is set as command-line args rather than in the kpopcoin.conf file.
        # This means that starting a kpopcoind using the temp dir to debug a failed test won't
        # spam debug.log.
    ...
    def __del__(self):
        # Ensure that we don't leave any kpopcoind processes lying around after
        # the test ends
        if self.process:
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then kpopcoind would bind P2P ports on
        ...
        # kpopcoin.conf. To avoid collisions, change it to 127.0.0.1:tor_port().
        ...
        # Add a new stdout and stderr file each time kpopcoind is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by kpopcoind, and
        ...
        self.running = True
        self.log.debug("kpopcoind started, waiting for RPC to come up")
    ...
    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the kpopcoind process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'kpopcoind exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                ...
                if error_num is None:
                    ...
                    # Windows can raise this while kpopcoind shuts down during startup.
                ...
                # Suppress if cookie file isn't generated yet and no rpcuser or rpcpassword; kpopcoind may be starting.
                if "No RPC credentials" not in str(e):
                    raise
                latest_error = suppress_error("missing_credentials", e)
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error(f"Unable to connect to kpopcoind after {self.rpc_timeout}s (ignored errors: {dict(suppressed_errors)!s}{'' if latest_error is None else f', latest: {latest_error[0]!r}/{latest_error[1]}'})")
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; kpopcoind is still starting
    ...
    def wait_until_stopped(self, *, timeout=KPOPCOIND_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to kpopcoind
        expected_msg: regex that stderr should match when kpopcoind fails

        Will raise if kpopcoind starts without an error.
        Will raise if an expected_msg is provided and it does not match kpopcoind's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'kpopcoind exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'kpopcoind should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to kpopcoin-cli for an individual node"""
    def __init__(self, binaries):
        self.options = []
        self.binaries = binaries
        self.input = None
        self.log = logging.getLogger('TestFramework.kpopcoincli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with kpopcoin-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run kpopcoin-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running kpopcoin-cli {}".format(p_args[2:]))
