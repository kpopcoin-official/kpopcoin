...
`test/functional/test_framework/test_framework.py:KpopcoinTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_kpopcoin_cli.py',
    'feature_bind_extra.py',
    'mempool_resurrect.py',
    'wallet_txn_doublespend.py --mineblock',
    'tool_kpopcoin_chainstate.py',
    'tool_wallet.py',
    'tool_utils.py',
...
def main():
    ...
    enable_kpopcoind = config["components"].getboolean("ENABLE_KPOPCOIND")

    if not enable_kpopcoind:
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if kpopcoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "kpopcoind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a kpopcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Kpopcoin functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/kpopcoin/kpopcoin/pull/27561.
...
class RPCCoverage():
    ...
    commands per `kpopcoin-cli help` (`rpc_interface.txt`).
