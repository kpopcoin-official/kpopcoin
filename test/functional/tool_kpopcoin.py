...
"""Test the kpopcoin wrapper tool."""
...
class ToolKpopcoinTest(KpopcoinTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `kpopcoin node -version` is
        # run on windows, python doesn't capture output from the child
        # `kpopcoind` and `kpopcoin-node` process started with _wexecvp, and
        # stdout/stderr are always empty. See
        # https://github.com/kpopcoin/kpopcoin/pull/33229#issuecomment-3265524908
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through kpopcoin wrapper command with specified arguments."""
        # Manually construct the `kpopcoin node` command, similar to Binaries::node_argv()
        kpopcoin_cmd = node.binaries.valgrind_cmd + [node.binaries.paths.kpopcoin_bin]
        node.args = kpopcoin_cmd + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure kpopcoin node command invokes kpopcoind by default")
        self.test_args([], [], expect_exe="kpopcoind")

        self.log.info("Ensure kpopcoin -M invokes kpopcoind")
        self.test_args(["-M"], [], expect_exe="kpopcoind")

        self.log.info("Ensure kpopcoin -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure kpopcoin -m invokes kpopcoin-node")
            self.test_args(["-m"], [], expect_exe="kpopcoin-node")

            self.log.info("Ensure kpopcoin -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="kpopcoin-node")

            self.log.info("Ensure kpopcoin accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="kpopcoin-node")

            self.log.info("Ensure kpopcoin recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="kpopcoin-node")
...
if __name__ == '__main__':
    ToolKpopcoinTest(__file__).main()
