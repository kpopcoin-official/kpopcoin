...
const char * const KPOPCOIN_CONF_FILENAME = "kpopcoin.conf";
const char * const KPOPCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //kpopcoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", KPOPCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\kpopcoin
    //   new: C:\Users\Username\AppData\Local\kpopcoin
    // macOS: ~/Library/Application Support/kpopcoin
    // Unix-like: ~/.kpopcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "kpopcoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "kpopcoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/kpopcoin";
#else
    // Unix-like
    return pathRet / ".kpopcoin";
#endif
#endif
}
