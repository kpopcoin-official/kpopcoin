...
/** Call node.updateRwSetting() with Kpopcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Kpopcoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/kpopcoin/kpopcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because kpopcoin 22.x will not try to read these.
...
//! Convert enabled/size values to kpopcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from kpopcoin -prune setting.
...
//! Get pruning size value to show in GUI from kpopcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayKpopcoinUnit")) {
        settings.setValue("DisplayKpopcoinUnit", QVariant::fromValue(KpopcoinUnit::BTC));
    }
    QVariant unit = settings.value("DisplayKpopcoinUnit");
    if (unit.canConvert<KpopcoinUnit>()) {
        m_display_kpopcoin_unit = unit.value<KpopcoinUnit>();
    } else {
        m_display_kpopcoin_unit = KpopcoinUnit::BTC;
        settings.setValue("DisplayKpopcoinUnit", QVariant::fromValue(m_display_kpopcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if kpopcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_kpopcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<KpopcoinUnit>() == m_display_kpopcoin_unit) return;
    m_display_kpopcoin_unit = new_unit.value<KpopcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayKpopcoinUnit", QVariant::fromValue(m_display_kpopcoin_unit));
    Q_EMIT displayUnitChanged(m_display_kpopcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/kpopcoin/kpopcoin/pull/8273
    ...
    // (https://github.com/kpopcoin-core/gui/issues/567).
