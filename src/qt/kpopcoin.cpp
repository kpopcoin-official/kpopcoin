...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<KpopcoinUnit>("KpopcoinUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. kpopcoin_de.qm (shortcut "de" needs to be defined in kpopcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. kpopcoin_de_DE.qm (shortcut "de_DE" needs to be defined in kpopcoin.qrc)
...
static const char* qt_argv = "kpopcoin-qt";

KpopcoinApplication::KpopcoinApplication()
...
void KpopcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the KpopcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", KpopcoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void KpopcoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool KpopcoinApplication::createOptionsModel(bool resetSettings)
{
...
void KpopcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new KpopcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &KpopcoinGUI::quitRequested, this, &KpopcoinApplication::requestShutdown);
...
void KpopcoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void KpopcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool KpopcoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void KpopcoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &KpopcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &KpopcoinApplication::handleRunawayException);
    connect(this, &KpopcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &KpopcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void KpopcoinApplication::parameterSetup()
{
...
void KpopcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void KpopcoinApplication::requestInitialize()
{
...
void KpopcoinApplication::requestShutdown()
{
...
void KpopcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    // kpopcoin: URIs or payment requests:
    if (paymentServer) {
        connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &KpopcoinGUI::handlePaymentRequest);
        connect(window, &KpopcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void KpopcoinApplication::handleRunawayException(const QString &message)
{
...
void KpopcoinApplication::handleNonFatalException(const QString& message)
{
...
WId KpopcoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool KpopcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", KpopcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(kpopcoin);
    Q_INIT_RESOURCE(kpopcoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(KPOPCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see kpopcoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see kpopcoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse kpopcoin.conf, determine network, switch to network specific
    ...
    // kpopcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
