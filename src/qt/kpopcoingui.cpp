 ...
 *
 * Ref: https://github.com/kpopcoin/kpopcoin/pull/1026
 */
static constexpr int64_t MAX_BLOCK_TIME_GAP = 90 * 60;

const std::string KpopcoinGUI::DEFAULT_UIPLATFORM =
...
KpopcoinGUI::KpopcoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &KpopcoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &KpopcoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &KpopcoinGUI::showModalOverlay);
...
void KpopcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Kpopcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kpopcoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &KpopcoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &KpopcoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &KpopcoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Kpopcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Kpopcoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Kpopcoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Kpopcoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a kpopcoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Kpopcoin command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &KpopcoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &KpopcoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &KpopcoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &KpopcoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &KpopcoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &KpopcoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &KpopcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &KpopcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &KpopcoinGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &KpopcoinGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &KpopcoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &KpopcoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &KpopcoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &KpopcoinGUI::showDebugWindow);
}

void KpopcoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void KpopcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &KpopcoinGUI::setCurrentWalletBySelectorIndex);
...
void KpopcoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &KpopcoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &KpopcoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &KpopcoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &KpopcoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void KpopcoinGUI::enableHistoryAction(bool privacy)
{
...
void KpopcoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &KpopcoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &KpopcoinGUI::removeWallet);
...
WalletController* KpopcoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void KpopcoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &KpopcoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &KpopcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &KpopcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &KpopcoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &KpopcoinGUI::incomingTransaction);
    connect(this, &KpopcoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void KpopcoinGUI::removeWallet(WalletModel* walletModel)
{
...
void KpopcoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void KpopcoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void KpopcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void KpopcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void KpopcoinGUI::createTrayIcon()
{
...
void KpopcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &KpopcoinGUI::toggleHidden);
...
void KpopcoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void KpopcoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void KpopcoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void KpopcoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void KpopcoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void KpopcoinGUI::openClicked()
{
...
void KpopcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void KpopcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void KpopcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void KpopcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void KpopcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void KpopcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void KpopcoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void KpopcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Kpopcoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void KpopcoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void KpopcoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void KpopcoinGUI::updateHeadersSyncProgressLabel()
{
...
void KpopcoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void KpopcoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &KpopcoinGUI::quitRequested);
...
void KpopcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void KpopcoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &KpopcoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void KpopcoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void KpopcoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &KpopcoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &KpopcoinGUI::show);
                e->ignore();
            }
        }
    }
...
void KpopcoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void KpopcoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void KpopcoinGUI::incomingTransaction(const QString& date, KpopcoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(KpopcoinUnits::formatWithUnit(unit, amount, true));
...
void KpopcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void KpopcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool KpopcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool KpopcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void KpopcoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void KpopcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void KpopcoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void KpopcoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void KpopcoinGUI::updateWindowTitle()
{
...
void KpopcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void KpopcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KpopcoinGUI::detectShutdown()
{
...
void KpopcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void KpopcoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

[[nodiscard]] static bool ThreadSafeMessageBox(KpopcoinGUI* gui, const bilingual_str& message, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = KpopcoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void KpopcoinGUI::subscribeToCoreSignals()
{
...
void KpopcoinGUI::unsubscribeFromCoreSignals()
{
...
bool KpopcoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<KpopcoinUnit> units = KpopcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const KpopcoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, KpopcoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const KpopcoinUnit u : KpopcoinUnits::availableUnits()) {
        menu->addAction(KpopcoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(KpopcoinUnit newUnits)
{
    setText(KpopcoinUnits::longName(newUnits));
}
