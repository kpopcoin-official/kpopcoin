...
//! Specify whether parameter should be parsed by kpopcoin-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make kpopcoin-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `kpopcoin-cli` attempt to parse the
 ...
 * `kpopcoin-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make kpopcoin-cli treat them as positional parameters when `-named` is used.
 * This prevents `kpopcoin-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so kpopcoin-cli
 ...
 *
 * For example, the command `kpopcoin-cli -named createwallet "my=wallet"`,
