...
namespace init {
namespace {
const char* EXE_NAME = "kpopcoind";

class KpopcoindInit : public interfaces::Init
{
public:
    KpopcoindInit(NodeContext& node) : m_node(node)
    {
        InitContext(m_node);
        m_node.init = this;
    }
...
{
    return std::make_unique<init::KpopcoindInit>(node);
}
} // namespace interfaces
