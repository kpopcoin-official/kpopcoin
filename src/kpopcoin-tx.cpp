...
static void SetupKpopcoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " kpopcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The kpopcoin-tx tool is used for creating and modifying kpopcoin transactions.\n\n"
                "kpopcoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded kpopcoin transaction, or with \"-create [commands]\" to create a hex-encoded kpopcoin transaction.\n"
                "\n"
                "Usage: kpopcoin-tx [options] <hex-tx> [commands]\n"
                "or:    kpopcoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded kpopcoin transaction
