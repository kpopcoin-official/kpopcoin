...
{
    // 10만 블록 단위로 감소 횟수(reductions) 계산
    int reductions = nHeight / consensusParams.nSubsidyHalvingInterval;

    // 133번째 감소(1,330만 블록)부터는 최소 보상 1 코인 고정
    if (reductions >= 133) {
        return 1 * COIN;
    }
    ...
    // 감소 횟수만큼 240/256 비율을 반복해서 곱해줌
    for (int i = 0; i < reductions; i++) {
        nSubsidy = (nSubsidy * 240) / 256;
    }

    // 혹시라도 계산 결과가 1 코인보다 작아지면 1 코인으로 강제 고정 (안전장치)
    if (nSubsidy < 1 * COIN) {
        return 1 * COIN;
    }
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // See https://github.com/kpopcoin/kpopcoin/issues/22596 for additional information.
...
bool IsBlockMutated(const CBlock& block, bool check_witness_root)
{
    ...
    if (block.vtx.empty() || !block.vtx[0]->IsCoinBase()) {
        ...
        // in "Weaknesses in Kpopcoin’s Merkle Root Construction":
        // https://lists.linuxfoundation.org/pipermail/kpopcoin-dev/attachments/20190225/a27d8837/attachment-0001.pdf).
        //
 ...
 *  grow our in-memory block index. See https://kpopcoincore.org/en/2024/07/03/disclose-header-spam.
 */
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // blocks to prevent timewarp attacks (see https://github.com/kpopcoin/kpopcoin/pull/15482).
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        // https://lists.linuxfoundation.org/pipermail/kpopcoin-dev/2019-February/016697.html.  Because CheckBlock() is
