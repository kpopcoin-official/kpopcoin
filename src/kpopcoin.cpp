...
Options:
  -m, --multiprocess     Run multiprocess binaries kpopcoin-node, kpopcoin-gui.
  -M, --monolithic       Run monolithic binaries kpopcoind, kpopcoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'kpopcoin-qt [ARGS]' or 'kpopcoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'kpopcoind [ARGS]' or 'kpopcoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'kpopcoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'kpopcoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'kpopcoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_kpopcoin [ARGS]'.
  chainstate [ARGS] Run kpopcoin kernel chainstate util, equivalent to running 'kpopcoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_kpopcoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_kpopcoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "kpopcoin-gui" : "kpopcoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "kpopcoin-node" : "kpopcoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("kpopcoin-cli");
            // Since "kpopcoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("kpopcoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("kpopcoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_kpopcoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("kpopcoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_kpopcoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_kpopcoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("kpopcoin-util");
        } else {
...
//! Execute the specified kpopcoind, kpopcoin-qt or other command line in `args`
...
//!             be a relative path to a kpopcoind, kpopcoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/kpopcoin ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/kpopcoin/kpopcoin/pull/31375#discussion_r1861814807)
