...
thread_local ThreadContext g_thread_context; // NOLINT(kpopcoin-nontrivial-threadlocal)
...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking kpopcoin code so they have to run
    ...
    // functions run, so client objects are fully disconnected before kpopcoin
    // code in the destructors are run. This way if the kpopcoin code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In kpopcoin core, running these callbacks in the right order is
